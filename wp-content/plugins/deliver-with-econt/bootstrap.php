...
function add_econt_payment_gateway($gateways) {
    if((!WC()->session || (@WC()->session->get( 'chosen_shipping_methods' )[0] == 'delivery_with_econt')) && get_woocommerce_currency() === "BGN") {
...
function track_deliver_with_econt_settings_changes()
{
    $oEcontPaymentSetttings = new Delivery_With_Econt_Payment();
    $oSettings = new Delivery_With_Econt_Helper();
    $isDemo = $oSettings->is_demo();
    $secret = DWEH()->get_private_key();
    $newValue = $oEcontPaymentSetttings->enabled;
    $newValue = ($newValue == 'yes') ? '1' : '0';
    $realUrl = 'https://delivery.econt.com/';
    $demoUrl = 'https://delivery-demo.econt.com/';
    ($isDemo) ? $url = $demoUrl : $ulr = $realUrl;
    $curl = curl_init();
    curl_setopt($curl, CURLOPT_URL, $url . "services/PluginsService.logEvent.json");
...
add_action('woocommerce_settings_saved', 'track_deliver_with_econt_settings_changes', 10, 1);
